...
from networks.RTNMADA import RTNMADA
from solvers.Solver import Solver
import torch.nn.functional as F


class RTNMADASolver(Solver):
    def __init__(self, dataset_type, source_domain, target_domain, cuda='cuda:0',
        ...
        super(RTNMADASolver, self).__init__(
        ...
        self.model_name = 'RTNMADA_LossWeight10.0_BCEWeight_noEntropy_TargetClassLoss_-+'
        self.iter_num = 0
    ...
    def set_model(self):
        if self.dataset_type == 'Digits':
            if self.task in ['MtoU', 'UtoM']:
                self.model = RTNMADA(n_classes=self.n_classes, base_model='DigitsMU')
            if self.task in ['StoM']:
                self.model = RTNMADA(n_classes=self.n_classes, base_model='DigitsStoM')

        if self.dataset_type in ['Office31', 'OfficeHome']:
            self.model = RTNMADA(n_classes=self.n_classes, base_model='ResNet50')
